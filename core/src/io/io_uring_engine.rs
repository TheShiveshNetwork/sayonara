...
impl IoUringEngine {
    ...
    pub fn write_at(&mut self, file: &File, data: &[u8], offset: u64) -> IOResult<usize> {
        ...
        if result < 0 {
            return Err(IOError::OperationFailed(
                format!("io_uring write failed: {}", std::io::Error::from_raw_os_error(-result))
            ));
        }
    ...
    pub fn read_at(&mut self, file: &File, buffer: &mut [u8], offset: u64) -> IOResult<usize> {
        ...
        if result < 0 {
            return Err(IOError::OperationFailed(
                format!("io_uring read failed: {}", std::io::Error::from_raw_os_error(-result))
            ));
        }
    ...
    pub fn batch_write(
        ...
        for (i, (offset, data)) in operations.iter().enumerate() {
            ...
            unsafe {
                self.ring
                    ...
                    .map_err(|e| IOError::OperationFailed(format!("io_uring batch push failed: {}", e)))?;
            }
        }

        // Submit all at once
        self.ring.submit()
            .map_err(|e| IOError::OperationFailed(format!("io_uring batch submit failed: {}", e)))?;
        ...
        for _ in 0..operations.len() {
            ...
            if let Some(cqe) = self.ring.completion().next() {
                ...
                if result < 0 {
                    return Err(IOError::OperationFailed(
                        format!("io_uring batch write failed: {}",
    ...
    pub fn batch_read(
        ...
        for _ in 0..operations.len() {
            ...
            if let Some(cqe) = self.ring.completion().next() {
                ...
                if result < 0 {
                    return Err(IOError::OperationFailed(
                        format!("io_uring batch read failed: {}",
                            std::io::Error::from_raw_os_error(-result))
                    ));
                }
                results.push(result as usize);
    ...
    pub fn fsync(&mut self, file: &File) -> IOResult<()> {
        ...
        unsafe {
            self.ring
                ...
                .map_err(|e| IOError::OperationFailed(format!("io_uring fsync push failed: {}", e)))?;
        }

        self.ring.submit_and_wait(1)
            .map_err(|e| IOError::OperationFailed(format!("io_uring fsync submit failed: {}", e)))?;
        ...
        if result < 0 {
            return Err(IOError::OperationFailed(
                format!("io_uring fsync failed: {}", std::io::Error::from_raw_os_error(-result))
            ));
        }
